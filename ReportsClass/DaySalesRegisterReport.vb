...
Public Class DaySalesRegisterReport
    ...
    Public Function ReportsDetails(ByVal SQLSTRING As String, ByVal PAGEHEADER() As String, ByVal FROMDATE As Date, ByVal TODATE As Date)
        ...
        Dim dblGrandPackamount, dblGrandRoundoff, dblDayPackamount, dblDayRoundoff As Double
        Dim dblDayPurchaserate, dblDayBillamt, dblDayTaxamt, dblDayTotalamt, dblDayTotal As Double
        Dim dblGrandPurchaserate, dblGrandBillamt, dblGrandTaxamt, dblGrandTotalamt, dblGrandTotal, dblGrandserchg, dblDayserchg As Double
        Dim POSDESC, SSQL As String
        Dim BILLDATE As Date
        Dim Billdatebool As Boolean
        Dim I, J As Integer
        Try
            ...
            If gdataset.Tables("DAYWISESALEREGISTER").Rows.Count > 0 Then
                ...
                For Each dr In gdataset.Tables("DAYWISESALEREGISTER").Rows
                    ...
                    If Format(DateValue(BILLDATE), "dd/MM/yyyy") <> Format(CDate(dr("BILLDATE")), "dd/MM/yyyy") Then
                        If Billdatebool = True Then
                            ...
                            pagesize = pagesize + 1
                            Filewrite.Write("{0,-16}{1,-20}{2,13}", "", "DAY TOTAL =====>", Mid(Format(Val(dblDayPurchaserate), "0.00"), 1, 13))
                            Filewrite.Write("{0,13}{1,13}{2,13}{3,13}", Mid(Format(Val(dblDayTaxamt), "0.00"), 1, 13), Mid(Format(Val(dblDayPackamount), "0.00"), 1, 13), Mid(Format(Val(dblDayserchg), "0.00"), 1, 13), Mid(Format(Val(dblDayTotal), "0.00"), 1, 13))
                            ...
                            BILLDATE = CDate(dr("BILLDATE"))
                            dblDayPurchaserate = 0 : dblDayTaxamt = 0 : dblDayTotalamt = 0 : dblDayTotal = 0
                            dblDayPackamount = 0 : dblDayRoundoff = 0 : dblDayserchg = 0
                            POSDESC = ""
                        Else
                            Filewrite.WriteLine("{0,-16}", Mid(Format(dr("BILLDATE"), "dd/MM/yyyy"), 1, 15))
                            pagesize = pagesize + 1
                    ...
                    If POSDESC <> Trim(CStr(dr("POSDESC"))) Then
                        Filewrite.Write("{0,-16}{1,-20}", "", Mid(CStr(dr("POSDESC")), 1, 20))
                        SSQL = "SELECT SUM(PURCHASERATE) AS PURCHASERATE,SUM(BILLAMOUNT) AS BILLAMOUNT,SUM(TAXAMOUNT) AS TAXAMOUNT,"
                        SSQL = SSQL & " SUM(PACKAMOUNT) AS PACKAMOUNT,SUM(SER_CHG) AS SER_CHG,SUM(ROUNDOFF) AS ROUNDOFF,0 AS DISCOUNT,SUM(TOTALAMOUNT) AS TOTALAMOUNT FROM VIEWSALEREGISTERDAYWISE "
                        ...
                        If gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows.Count > 0 Then
                            ...
                            Filewrite.Write("{0,13}", Mid(Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("SER_CHG")), "0.00"), 1, 12))
                            dblDayserchg = dblDayserchg + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("SER_CHG")), "0.00")
                            dblGrandserchg = dblGrandserchg + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("SER_CHG")), "0.00")
                            ''-----------------
                            '''''Filewrite.Write("{0,13}", Mid(Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("BILLAMOUNT")), "0.00"), 1, 12))
                            '''''dblDayTotalamt = dblDayTotalamt + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("BILLAMOUNT")), "0.00")
                            '''''dblGrandTotalamt = dblGrandTotalamt + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("BILLAMOUNT")), "0.00")
                            ...
                            ''''dblDayTotal = dblDayTotal + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("TOTALAMOUNT")), "0.00")
                            ''''dblGrandTotal = dblGrandTotal + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("TOTALAMOUNT")), "0.00")
                            Filewrite.Write("{0,13}", Mid(Format(Val(Math.Round(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("BILLAMOUNT"))), "0.00"), 1, 12))
                            dblDayTotalamt = dblDayTotalamt + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("BILLAMOUNT")), "0.00")
                            dblGrandTotalamt = dblGrandTotalamt + Format(Val(gdataset.Tables("VIEWSALEREGISTERDAYWISE").Rows(0).Item("BILLAMOUNT")), "0.00")
                            Filewrite.WriteLine()
                            pagesize = pagesize + 1
                ...
                Filewrite.WriteLine(StrDup(135, "."))
                pagesize = pagesize + 1
                'Filewrite.Write("{0,-16}{1,-13}{2,13}", "", "DAY TOTAL =====>", Mid(Format(Val(dblDayPurchaserate), "0.00"), 1, 13))
                ...
                Filewrite.Write("{0,13}", Format(Val(dblGrandPurchaserate), "0.00"))
                Filewrite.Write("{0,13}", Format(Val(dblGrandTaxamt), "0.00"))
                Filewrite.Write("{0,13}", Format(Val(dblGrandPackamount), "0.00"))
                Filewrite.Write("{0,13}", Format(Val(dblGrandserchg), "0.00"))
                Filewrite.Write("{0,13}", Format(Val(dblGrandTotal), "0.00"))
                Filewrite.Write("{0,2}", "")
                Filewrite.Write("{0,13}", Format(Val(dblGrandRoundoff), "0.00"))
                Filewrite.Write("{0,13}", Format(Math.Round(Val(dblGrandTotalamt)), "0.00"))
                Filewrite.WriteLine()
                pagesize = pagesize + 1
                Filewrite.WriteLine(StrDup(135, "="))
                pagesize = pagesize + 1
    ...
    Private Function PrintHeader(ByVal HEADING() As String, ByVal MSKFROMDATE As Date, ByVal MSKTODATE As Date)
        ...
        Try
            Filewrite.WriteLine("{0,-65}", Chr(14) & Chr(15) & " " & Mid(MyCompanyName, 1, 65) & Chr(15))
            Filewrite.WriteLine("{0,-80}", "DAYWISE SALES REGISTER FROM:" & Format(MSKFROMDATE, "dd/MM/yyyy") & " TO:" & Format(MSKTODATE, "dd/MM/yyyy") & Space(50) & "PAGE : " & pageno)
            Filewrite.WriteLine(StrDup(135, "-"))
            pagesize = pagesize + 1
            Filewrite.WriteLine("{0,-16}{1,-20}{2,13}{3,13}{4,13}{5,13}{6,13}{7,13}{8,13}", "BILL", "COST", "PURCHASE", "VAT", "AC.CHG", "KICH.CHG", "BASIC", "ROUNDOFF", "BILL")
            pagesize = pagesize + 1
            Filewrite.WriteLine("{0,-16}{1,-20}{2,13}{3,13}{4,13}{5,13}{6,13}{7,13}{8,13}", "DATE", "CENTRE", "AMOUNT", "", "", "", "AMOUNT", "AMOUNT", "AMOUNT")
            pagesize = pagesize + 1
            Filewrite.WriteLine(StrDup(135, "-"))
            pagesize = pagesize + 1
        Catch ex As Exception
