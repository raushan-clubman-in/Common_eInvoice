...
Public Class PendingKOTRegisterReport
        ...
        Try
            ...
            If gdataset.Tables("PENDINGKOTREGISTER").Rows.Count > 0 Then
                ...
                For Each dr In gdataset.Tables("PENDINGKOTREGISTER").Rows
                    ...
                    Filewrite.WriteLine("{0,10}{1,12}{2,12}", Mid(Format(Val(dr("RATE")), "0.00"), 1, 10), Mid(Format(Val(dr("TAXAMOUNT")), "0.00"), 1, 10), Mid(Format(Val(dr("AMOUNT")), "0.00"), 1, 10))
                    pagesize = pagesize + 1
                    KOTAmount = KOTAmount + Format(Val(dr("AMOUNT")), "0.00")
                    GroupAmount = GroupAmount + Format(Val(dr("AMOUNT")), "0.00") + Format(Val(dr("TAXAMOUNT")), "0.00")
                Next dr
                Filewrite.WriteLine()
                pagesize = pagesize + 1
                ''''************************************** $  ITEM GROUP TOTAL $   **********************************''''
    ...
    Private Function PrintHeader(ByVal Heading() As String, ByVal mskfromdate As Date, ByVal msktodate As Date)
        ...
        Try
            ...
            Filewrite.WriteLine("{0,-10}{1,10}{2,12}{3,12}", "UOM", "RATE", PrintTaxheading1, "AMOUNT")
            pagesize = pagesize + 1
            Filewrite.Write("{0,-20}{1,-20}{2,-15}{3,-25}{4,-10}", "BILL DATE", "NUMBER", "CODE", "DESCRIPTION", "")
            Filewrite.WriteLine("{0,-10}{1,10}{2,12}{3,12}", "", "", PrintTaxheading2, "")
            pagesize = pagesize + 1
            Filewrite.WriteLine(StrDup(135, "-"))
            pagesize = pagesize + 1
        Catch ex As Exception
