...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.TXT_NEWPASSWORD.Location = New System.Drawing.Point(151, 104)
        ...
        Me.TXT_NEWPASSWORD.TabIndex = 605
        ...
        Me.Cmd_Cancel.Location = New System.Drawing.Point(160, 136)
        ...
        Me.Cmd_Login.Location = New System.Drawing.Point(64, 136)
        ...
        Me.Label4.Location = New System.Drawing.Point(104, 231)
        ...
        Me.Label5.Location = New System.Drawing.Point(704, 448)
        ...
        Me.Label7.Text = "Copyrights Reserved 2004-2012  by CLUBMAN CHENNAI"
        '
        'PictureBox2
        '
        Me.PictureBox2.Image = CType(resources.GetObject("PictureBox2.Image"), System.Drawing.Image)
        Me.PictureBox2.Location = New System.Drawing.Point(120, 346)
        Me.PictureBox2.Name = "PictureBox2"
        Me.PictureBox2.Size = New System.Drawing.Size(232, 54)
...
#End Region
    ...
    Private Sub LoginForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        gconnection.FileInfo(Application.StartupPath & "\SMARTPOS.Exe")
        ...
        'SMARTCARD
        Call GetPHOTOPATH()
        getdashdata(sender, e)
    End Sub
    Private Sub getdashdata(ByVal SENDER, ByVal E)
    ...
    Private Sub Cmd_Login_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Login.Click
        ...
        Try
            ...
            ' SQLSTRING = "SELECT * FROM USERADMIN WHERE  USERNAME = '" & Trim(UCase(Txt_Username.Text)) & "' and USERPASSWORD ='" & Trim(GetPassword(UCase(Txt_Password.Text))) & "' AND MAINGROUP = 'ACCOUNTS INTEGRATION' AND CATEGORY  = 'U'"
            ' Else
    ...
    Public Sub GetServer()
        ...
        Try
            ...
            ServerConn.Open()
            Dim sqlstring As String
            ssql = "SELECT SERVER,SqlUserName,SqlUserPassword FROM DBSKEY"
            servercmd = New OleDb.OleDbDataAdapter(ssql, ServerConn)
            servercmd.Fill(getserver)
            ...
            If getserver.Tables(0).Rows.Count > 0 Then
                ...
                'for tally
                'gTallyOdbc = Trim(getserver.Tables(0).Rows(0).Item(1) & "")
                'gTallyPort = Trim(getserver.Tables(0).Rows(0).Item(2) & "")
                'gTallyDsn = Trim(getserver.Tables(0).Rows(0).Item(3) & "")
                'gTallyIntegration = Trim(getserver.Tables(0).Rows(0).Item(4) & "")

                'gTallyKotFlag = Trim(getserver.Tables(0).Rows(0).Item(5) & "")
                'gTallyFBillFlag = Trim(getserver.Tables(0).Rows(0).Item(6) & "")
                'gTallyDBillFlag = Trim(getserver.Tables(0).Rows(0).Item(7) & "")
                'gTallyBatchFlag = Trim(getserver.Tables(0).Rows(0).Item(8) & "")

                'gTallyCompanyName = Trim(getserver.Tables(0).Rows(0).Item("TallyCompanyName") & "")
                'gTallyServer = Trim(getserver.Tables(0).Rows(0).Item(10) & "")
                'gTallyUser = Trim(getserver.Tables(0).Rows(0).Item(11) & "")
                gSqlUserName = Trim(getserver.Tables(0).Rows(0).Item(1) & "")
                'gSqlUserPassword = abcdMINUS(Trim(getserver.Tables(0).Rows(0).Item(13) & ""))
                gSqlUserPassword = Trim(getserver.Tables(0).Rows(0).Item(2) & "")

                'gTallymonthbill = Trim(getserver.Tables(0).Rows(0).Item("Tally_monthbill") & "")
                '''''gTallymonthbill = "Gandhi Nagar Club"

                'If gSqlUserPassword = Nothing Then
                '    gSqlUserPassword = ""
                'End If

                ''''gRoundOffFlag = Trim(getserver.Tables(0).Rows(0).Item(14) & "")
                'gClosingBalanceFlag = Trim(getserver.Tables(0).Rows(0).Item("ClosingBalanceFlag") & "")
                'gProductKey = Trim(getserver.Tables(0).Rows(0).Item(16) & "")
                'gTallyBulkUpdateFBill = Trim(getserver.Tables(0).Rows(0).Item(17) & "")
                'gTallyBulkUpdateKOT = Trim(getserver.Tables(0).Rows(0).Item(18) & "")
                'gKotWithBill = Trim(getserver.Tables(0).Rows(0).Item(19) & "")
                'gTcpProvider = Trim(getserver.Tables(0).Rows(0).Item("TCPPROVIDER") & "")

                'gacessFlag = Trim(getserver.Tables(0).Rows(0).Item(20) & "")
                'MsgBox(abcdADD(Environment.MachineName))
                'If abcdMINUS(Trim(gProductKey)) <> Trim(Environment.MachineName) Then
                Call Randomize()
                AppPath = Application.StartupPath
                vOutfile = Mid("Pwd" & (Rnd() * 800000), 1, 8)
                ...
                Dim lic
                'lic = abcdADD(Trim(Environment.MachineName))
                ' Txt_Username.Text = lic

                ' MsgBox(lic)
                'MsgBox(lic)

                'Filewrite.WriteLine(lic)
                ' Filewrite.WriteLine(Trim(Environment.MachineName))
                ' Filewrite.Close()
                ...
                'ExcelConn.Open()
                'ssql = "UPDATE EXCELPATH SET PWD='" & lic & "'"
                'Excelcmd = New OleDb.OleDbDataAdapter(ssql, ExcelConn)
                'Excelcmd.UpdateCommand(ssql, ExcelConn)


                'MsgBox("Get License from DBS.....info@databasesoftware.in")
            ...
            Else
                ...
                'for tally
                gTallyOdbc = "http://localhost:"
                gTallyPort = 9000
                gTallyDsn = "DSN=TallyODBC_9000"
                gTallyIntegration = "Y"
                gTallyKotFlag = ""
                gTallyFBillFlag = ""
                gTallyDBillFlag = ""
    ...
    Private Sub LoginForm_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            If MsgBox("Close this module...", MsgBoxStyle.OKCancel + MsgBoxStyle.DefaultButton1, "CLOSE") = MsgBoxResult.OK Then
                Me.Close()
                End
            End If
        End If
