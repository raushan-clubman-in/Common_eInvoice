Option Explicit On 
Public Class ListOperattion_MEMBER
...
#End Region
    ...
    Private Sub getDetails()
        Try
            ...
            keyfield14 = POSListoperation.get_TextMatrix(POSListoperation.Row, keypos14)
            keyfield15 = POSListoperation.get_TextMatrix(POSListoperation.Row, Keypos15)
    ...
    Private Sub txtSearch_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSearch.TextChanged
        Try
            ...
            If CbxColumn.Text <> "" Then
                ...
                ssql = ssql & " (MCODE LIKE '%" & Trim(txtSearch.Text) & "%' OR MNAME LIKE '%" & Trim(txtSearch.Text) & "%')"
            ...
            If ssql <> "" Then
                ...
                If gdataset.Tables("MyTable").Rows.Count > 0 Then
                    ...
                    Next
                Else
                    'If Mid(gTallyIntegration, 1, 1) = "Y" And Mid(gTallyFBillFlag, 1, 1) = "Y" Then
                    '    Dim myconnection As New ADODB.Connection
                    '    Dim ssql As String
                    '    Dim vfill As New ADODB.Recordset

                    '    Dim MCODE, MNAME, PEMAIL, PSTATE, PPIN, PPHONE1, PFAX, PMOBILE, PADD1, PADD2, PADD3, PCITY, MEMBERTYPECODE As String
                    '    Dim CREDITLIMIT As Double

                    '    myconnection.Open(gTallyDsn)
                    '    ssql = "Select $name,$email,$statename,$pincode,$ledgerphone,$ledgerfax,$ledgercontact,$creditlimit,$address,$address1,$address2,$address3,$address4,$address5,$parent,$additionalname from '" & gTallyCompanyName & "'.'" & gTallyUser & "'.'Ledger' where $$IsBelongsTo:$$GroupSundryDebtors and ($Name='" & Trim(txtSearch.Text) & "' OR $Name='" & Trim(txtSearch.Text) & "')"
                    '    vfill.Open(ssql, myconnection)
                    '    If Not vfill.EOF Then
                    '        MsgBox("Ledger is availbale in Tally Kindly Create", MsgBoxStyle.OKOnly, "Item")
                    '        'vfill.Fields.Item("$additionalname").Value
                    '        If IsDBNull(vfill.Fields.Item("$name").Value) = False Then
                    '            MCODE = vfill.Fields.Item("$name").Value
                    '        Else
                    '            MCODE = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$additionalname").Value) = False Then
                    '            MNAME = vfill.Fields.Item("$additionalname").Value
                    '        Else
                    '            MNAME = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$email").Value) = False Then
                    '            PEMAIL = vfill.Fields.Item("$email").Value
                    '        Else
                    '            PEMAIL = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$statename").Value) = False Then
                    '            PSTATE = vfill.Fields.Item("$statename").Value
                    '        Else
                    '            PSTATE = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$pincode").Value) = False Then
                    '            PPIN = vfill.Fields.Item("$pincode").Value
                    '        Else
                    '            PPIN = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$ledgerphone").Value) = False Then
                    '            PPHONE1 = vfill.Fields.Item("$ledgerphone").Value
                    '        Else
                    '            PPHONE1 = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$ledgerfax").Value) = False Then
                    '            PFAX = vfill.Fields.Item("$ledgerfax").Value
                    '        Else
                    '            PFAX = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$ledgercontact").Value) = False Then
                    '            PMOBILE = vfill.Fields.Item("$ledgercontact").Value
                    '        Else
                    '            PMOBILE = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$creditlimit").Value) = False Then
                    '            CREDITLIMIT = vfill.Fields.Item("$creditlimit").Value
                    '        Else
                    '            CREDITLIMIT = 0
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$address1").Value) = False Then
                    '            PADD1 = vfill.Fields.Item("$address1").Value
                    '        Else
                    '            PADD1 = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$address2").Value) = False Then
                    '            PADD2 = vfill.Fields.Item("$address2").Value
                    '        Else
                    '            PADD2 = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$address3").Value) = False Then
                    '            PADD3 = vfill.Fields.Item("$address3").Value
                    '        Else
                    '            PADD3 = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$address4").Value) = False Then
                    '            PCITY = vfill.Fields.Item("$address4").Value
                    '        Else
                    '            PCITY = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$address5").Value) = False Then
                    '            PSTATE = vfill.Fields.Item("$address5").Value
                    '        Else
                    '            PSTATE = ""
                    '        End If
                    '        If IsDBNull(vfill.Fields.Item("$parent").Value) = False Then
                    '            MEMBERTYPECODE = vfill.Fields.Item("$parent").Value
                    '        Else
                    '            MEMBERTYPECODE = ""
                    '        End If
                    '        ssql = "INSERT INTO MEMBERMASTER (MCODE,MNAME,PEMAIL,PSTATE,PPIN,PPHONE1,PFAX,PCELL,CREDITLIMIT,PADD1,PADD2,PADD3,PCITY,PSTATE,MEMBERTYPECODE) VALUES ('" & Trim(MCODE) & "','" & Trim(MNAME) & "','" & Trim(PEMAIL) & "','" & Trim(PSTATE) & "','" & Trim(PPIN) & "','" & Trim(PPHONE1) & "','" & Trim(PFAX) & "','" & Trim(PMOBILE) & "'," & Format(CREDITLIMIT, "0.00") & ",'" & Trim(PADD1) & "','" & Trim(PADD2) & "','" & Trim(PADD3) & "','" & Trim(PCITY) & "','" & Trim(PSTATE) & "','" & Trim(MEMBERTYPECODE) & "')"
                    '        gconnection.dataOperation(6, ssql, "MEMBER")
                    '    Else
